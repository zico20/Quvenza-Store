  ...
  return (
    <div style={{ background: '#17171a', border: '1px solid #2a2a30', borderRadius: 6, padding: 24 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', gap: 2, padding: 2, background: '#0e0e10', border: '1px solid #2a2a30', borderRadius: 4 }}>
          {['7D', '30D', '90D'].map((p, i) => (
            <button key={p} style={{
              padding: '4px 10px', fontSize: 11, fontFamily: 'JetBrains Mono, monospace',
              background: i === 1 ? '#17171a' : 'transparent',
              color: i === 1 ? '#f5f5f4' : '#6b6b70',
      ...
      <ResponsiveContainer width="100%" height={180}>
        <BarChart data={data} margin={{ top: 0, right: 0, left: 0, bottom: 0 }} barCategoryGap="20%">
          ...
          <XAxis
            dataKey="date"
            tick={{ fontSize: 10, fill: '#6b6b70', fontFamily: 'JetBrains Mono, monospace' }}
          ...
          />
          <Tooltip
            contentStyle={{ backgroundColor: '#1f1f23', border: '1px solid #2a2a30', borderRadius: 6, fontSize: 12, color: '#f5f5f4' }}
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'flex-end', height: 180, gap: 3, marginTop: -180 }}>
      {bars.map((h, i) => (
        <div key={i} style={{
          flex: 1, height: `${h}%`,
          background: i === bars.length - 1 ? '#ff6a2b' : '#1f1f23',
