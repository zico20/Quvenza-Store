  ...
  return (
    <>
      ...
      <div style={{ maxWidth: 800, margin: '0 auto', padding: '64px 24px' }}>
        <h1 style={{ fontSize: 40, fontWeight: 700, color: '#F7F7F8', marginBottom: 12, letterSpacing: '-0.02em' }}>طرق الدفع المتاحة</h1>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 16, marginBottom: 48 }}>
          {methods.map((m) => (
            <div key={m.name} style={{ background: '#121216', border: '1px solid #26262E', borderRadius: 8, padding: 24 }}>
              <div style={{ fontSize: 40, marginBottom: 16 }}>{m.icon}</div>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 8 }}>
                <h2 style={{ fontSize: 20, fontWeight: 700, color: '#F7F7F8', margin: 0 }}>{m.name}</h2>
                <span style={{ fontSize: 12, color: '#6C6C76' }}>({m.nameEn})</span>
              </div>
              <p style={{ color: '#A6A6AE', fontSize: 14, lineHeight: 1.65, marginBottom: 12 }}>{m.desc}</p>
        ...
        <div style={{ background: '#121216', border: '1px solid #26262E', borderRadius: 8, padding: 32, marginBottom: 24 }}>
          <h2 style={{ fontSize: 20, fontWeight: 600, color: '#F7F7F8', marginBottom: 16 }}>ماذا لا نقبل؟</h2>
          ...
          <p style={{ color: '#6C6C76', fontSize: 13, marginTop: 16 }}>
