      ...
      <div style={{ padding: 32 }}>
        ...
        {loading ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {[...Array(6)].map((_, i) => (
              <div key={i} style={{ height: 60, background: '#FFFFFF', border: '1px solid #EAECEF', borderRadius: 4 }} />
            ))}
          </div>
        ) : filtered.length === 0 ? (
          <div style={{ background: '#FFFFFF', border: '1px solid #EAECEF', borderRadius: 6, padding: '64px 32px', textAlign: 'center' }}>
            <Icon name="package" size={40} stroke={1.2} style={{ color: '#9097A1', marginBottom: 16 }} />
            <p style={{ color: '#111827', fontSize: 16, fontWeight: 600, margin: 0 }}>{t('products.noProducts')}</p>
          </div>
        ) : (
          <div style={{ background: '#FFFFFF', border: '1px solid #EAECEF', borderRadius: 6, overflow: 'hidden' }}>
            ...
            <div style={{
              ...
              padding: '10px 20px', background: '#F3F4F6', borderBottom: '1px solid #EAECEF', gap: 12,
            }}>
              ...
              return (
                ...
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    <div style={{ width: 40, height: 40, borderRadius: 4, overflow: 'hidden', flexShrink: 0, background: p.images?.[0] ? '#F3F4F6' : placeholderBg }}>
                      {p.images?.[0] && (
                    ...
                    <div>
                      ...
                      <div className="mono" style={{ fontSize: 10, color: '#9097A1' }}>{p.slug}</div>
                    </div>
                  </div>
                  {/* Category */}
                  <div style={{ fontSize: 12, color: '#4B5563' }}>{(p as any).category?.name ?? '—'}</div>
                  {/* Price */}
                  <div style={{ fontSize: 13, fontWeight: 600, color: '#111827' }}>{formatPrice(p.price)}</div>
                  {/* Stock */}
                  <div style={{ fontSize: 13, fontWeight: 600, color: p.stock === 0 ? '#EF4444' : p.stock <= 10 ? '#F59E0B' : '#16A34A' }}>
                    {p.stock}
                  </div>
                  {/* Status */}
                  <div>
                  ...
                  <div style={{ display: 'flex', gap: 4 }}>
                    <Link href={`/admin/dashboard/products/${p.id}`} style={{
                      ...
                      background: 'transparent', color: '#4B5563', border: 'none', cursor: 'pointer', textDecoration: 'none',
                    }}>
        ...
        {/* Count */}
        <p className="mono" style={{ fontSize: 10, color: '#9097A1', marginTop: 12 }}>
