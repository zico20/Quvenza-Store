...
export async function initiatePayment(req: Request, res: Response, next: NextFunction) {
  try { sendSuccess(res, await paymentService.initiatePayment(req.body.orderId, req.user!.id, req.body.currency)); } catch (e) { next(e); }
}
export async function confirmPayment(req: Request, res: Response, next: NextFunction) {
  try { sendSuccess(res, await paymentService.confirmOrderPayment(req.body.orderId, req.user!.id, req.body.paymentIntentId)); } catch (e) { next(e); }
}
